...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post('/api/proxy/for4payments/pix', async (req, res) => {
    try {
      ...
      const { name, cpf, email, phone, amount = 67.90, description = "Kit de Segurança Shopee Delivery" } = req.body;
  ...
  app.post('/api/payments/pix', async (req, res) => {
    try {
      ...
      // Valor fixo para o kit de segurança: R$ 67,90
      const paymentAmount = 67.90;
